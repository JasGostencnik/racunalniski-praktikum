...
\subsection{Vhodni podatki}
   ...
   Da bi lahko program sploh začel z delom in poskušal iz danih podatkov
 izdelati urnik, v prvi vrsti potrebujemo podatke, ki jih moramo upoštevati in
...
\begin{itemize}
      ...
      \item Razred \emph{Predavalnica} nosi podatke o kapaciteti predavalnice, 
      njeno oznako ter podatke o zasedenosti, ki so predstavljeni s tabelo.
...
\[
velikostTabele = \text{številoUrNaDan} \cdot \text{številoDelovnihDni}
\]
...
kršitev omejitve, z $u_n$ pa njeno utež. Z naslednjo formulo izračunamo
kakovost urnika:
\[
...
\begin{enumerate}
      ...
      označimo s \text{predmet\textsubscript{$i$}}.
   
      \item  Naslednja vgnezdena zanka nas popelje po vseh predavalnicah. Predavalnico na
      $j$-tem koraku označimo s \text{predavalnica\textsubscript{$j$}}.
   
      \item Za vse proste termine \text{predavalnica\textsubscript{$j$}} izračunamo ceno vstavljanja
      \text{srečanja\textsubscript{$i$}}, ki ga sestavlja \text{predmet\textsubscript{i}}, profesor, ki ta predmet predava,
      ...
      vstavljanja, da bomo \text{predmet\textsubscript{i}} na koncu vstavili v termin z najboljšo
      ceno vstavljanja.
   
      \item  Ko preverimo vse proste termine vseh predavalnic, vstavimo \text{srečanja\textsubscript{$i$}}
...
spremembo (npr. zamenjava \text{srečanja\underline{$i$}} in \text{srečanja\underline{$j$}}) sedaj shranimo v
